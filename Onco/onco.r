...
if (0){    
  ## Antes retirar espaços e converter para CAIZA ALTA
  Tumor <- base$onco %>% count(TumorPrim) %>% arrange(desc(n)) %>%
    mutate(TumorPrimNovo = word(TumorPrim,-1,-1)) %>%
  mutate(TumorPrimNovo = replace(TumorPrimNovo, TumorPrim == "Medula óssea","Medula"))
  ### Falta limpar alguns dados e aplicar à tabela original
  view(Tumor)
   }
